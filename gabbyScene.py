...
#doesnt work - issues with locations and might be harder to do what we want
class PixelArray(Scene):
    def construct(self):
        ...
        for x in np.arange(-1.5, 1.5, 1.0):
            for y in np.arange(-1.5,1.5,1.0):
                locations[f"{x}{y}"] = TexMobject(f"({x}{y})")
                dots[f"{var_index}"] = Dot(np.array([x,y,0]),radius=.20)
                var_index = var_index+1
        for dot in dots.values():
            dot.set_color(C.GREEN_A)
            self.add(dot)
            self.wait(0.5)
